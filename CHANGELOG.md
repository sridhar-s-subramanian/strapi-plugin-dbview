...
- **Database Browser** table headers now use the full cell width. The column
  label and sort controls stay on the left, while filter and foreign-key
  indicators are grouped on the right, eliminating the large empty gap on the
  right side of wide headers.

### Changed

- **Database Browser** filter button now uses the `ghost` IconButton variant
  when no filter is applied and the filled `default` variant when a filter is
  active, making active filters easier to spot.
